...
- **소셜**
  ...
  - 일반 사용자에게서 구글, 네이버 등 자신의 소셜 타입을 회원 조회 시 알 수 있으면 좋을 거 같다는 의견을 받아 회원 조회 시 기본 회원 아이콘 대신 소셜 아이콘을 노출하도록 변경했습니다.
...
- **날짜 범위 조회**
  - 일반 사용자의 피드백을 통해 직접 날짜 범위를 지정해서 수입을 전체 조회할 수 있는 기능을 추가 했습니다.
  - 시작 날짜와 종료 날짜를 받아 해당 날짜에 포함된 모든 수입을 조회합니다.
  - 모든 날짜는 파라미터로 받아 문자열에서 Java의 LocalDate 로 변환 후 조회합니다.
- **수입 태그**
